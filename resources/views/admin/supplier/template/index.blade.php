...
@section('content')
    <div class="container pd-x-0">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-5">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Approver Template</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Manage Approver Template</h4>
            </div>
        </div>
        ...
        <div class="row row-sm">
                ...
                <div class="filter-buttons mg-b-10">
                    <div class="d-md-flex bd-highlight">
                        <div class="bd-highlight mg-r-10 mg-t-10">
                            ...
                            @if (auth()->user()->has_access_to_route('supplier-categories.delete'))
                        ...
                        <div class="ml-auto bd-highlight mg-t-10">
                            <form class="form-inline" id="searchForm">
                                ...
                                @if (auth()->user()->has_access_to_route('approver-steps.create'))
                                    <a class="btn btn-primary btn-sm mg-b-5" href="{{ route('approver-steps.create') }}">Create a Approver Step</a>
                                @endif
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive-lg">
                        <table class="table mg-b-0 table-light table-hover">
                            ...
                            <tr>
                                ...
                                <th scope="col" width="25%">Sequence</th>
                                <th scope="col" width="15%">Can override</th>
                                <th scope="col" width="10%">Options</th>
                            </tr>
                            </thead>
                            <tbody>
                            @forelse($steps as $step)
                                <tr id="row{{$step->id}}" class="row_cb">
                                    <th>
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input cb" id="cb{{ $step->id }}">
                                            <label class="custom-control-label" for="cb{{ $step->id }}"></label>
                                        </div>
                                    </th>
                                    <td><strong> {{ $step->user->username }}</strong></td>
                                  
                                    <td>{{ $step->sequence_no }}</td>
                                    <td>{{ !is_null($step->overridable) ? $step->overridable:"not allowed to override" }}</td>
                                    <td>                                        
                                        
                                        @if($step->trashed())
                                            <nav class="nav table-options">
                                                <a class="nav-link" href="{{route('approver-steps.restore',$step->id)}}" title="Restore this step"><i data-feather="rotate-ccw"></i></a>
                                            </nav>
                                        @else
                                            <nav class="nav table-options">
                                                @if (auth()->user()->has_access_to_route('supplier-categories.edit'))
                                                    <a class="nav-link" href="{{ route('approver-steps.edit',$step->id) }}" title="Edit Step"><i data-feather="edit"></i></a>
                                                @endif
                                                @if (auth()->user()->has_access_to_route('approver-steps.delete'))
                                                    <a class="nav-link" href="javascript:void(0)" onclick="delete_one_category({{$step->id}})" title="Delete Step"><i data-feather="trash"></i></a>
                                                @endif
                                            </nav>     
                                        @endif
                            ...
                            @empty
                                <tr>
                                    <th colspan="5" style="text-align: center;"> <p class="text-danger">No Approver Steps found.</p></th>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
            ...
            <div class="col-md-6">
                <div class="mg-t-5">
                    @if ($steps->firstItem() == null)
                        <p class="tx-gray-400 tx-12 d-inline">{{__('common.showing_zero_items')}}</p>
                    @else
                        <p class="tx-gray-400 tx-12 d-inline">Showing {{($steps->firstItem() ?? 0)}} to {{($steps->lastItem() ?? 0)}} of {{$steps->total()}} items</p>
                    @endif
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-md-right float-md-right mg-t-5">
                    {{ $steps->appends((array) $filter)->links() }}
                </div>
            </div>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('approver-steps.index') }}";
    ...
    <script>
        ...
        function delete_category(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('approver-steps.delete')}}",'',selected_pages);
                });
        ...
        function delete_one_category(id){
            ...
            $('#btnDelete').on('click', function() {
                post_form('{{route('approver-steps.delete')}}','',id);
            });
        }
    </script>
@endsection
