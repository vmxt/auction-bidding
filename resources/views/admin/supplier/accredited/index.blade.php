                ...
                <div class="filter-buttons mg-b-10">
                    <div class="d-md-flex bd-highlight">
                        <div class="bd-highlight mg-r-10 mg-t-10">
                            ...
                            @if (auth()->user()->has_access_to_route('suppliers.delete'))
                        ...
                        <div class="ml-auto bd-highlight mg-t-10">
                            <form class="form-inline" id="searchForm">
                                ...
                                @if (auth()->user()->has_access_to_route('suppliers.create'))
                                    <a class="btn btn-primary btn-sm mg-b-5" href="{{ route('suppliers.create') }}">Invite Supplier</a>
                                @endif
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive-lg">
                        <table class="table mg-b-0 table-light table-hover" style="width:100%;">
                            ...
                            @forelse($suppliers as $supplier)
                                ...
                                @if(!empty($supplier->supplier_details->company_name))

                                    <tr id="row{{$supplier->id}}" class="row_cb">
                                        <td>
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input cb" id="cb{{ $supplier->id }}">
                                                <label class="custom-control-label" for="cb{{ $supplier->id }}"></label>
                                            </div>
                                        </td>
                                        <td><strong @if($supplier->trashed()) style="text-decoration:line-through;" @endif> {{ $supplier->supplier_details->company_name }}</strong></td>                                      
                                        <td>{{ $supplier->supplier_details->Address }}</td>
                                        <td>{{ $supplier->supplier_details->business_type }}</td>
                                        <td>{{ $supplier->supplier_details->status }}</td>
                                        <td>                                        
                                            <nav class="nav table-options">

                                                @if($supplier->trashed())
                                                    @if (auth()->user()->has_access_to_route('suppliers.restore'))
                                                        <nav class="nav table-options">
                                                            <a class="nav-link" href="{{route('suppliers.restore',$supplier->id)}}" title="Restore this category"><i data-feather="rotate-ccw"></i></a>
                                                        </nav>
                                                    @endif
                                                @else
                                                    <a class="nav-link" href="{{route('sms.auth.profile.view',$supplier->id)}}" target="_blank" title="View Profile"><i data-feather="eye"></i></a>
                                                    @if (auth()->user()->has_access_to_route('suppliers.edit'))
                                                        <a class="nav-link" href="{{ route('suppliers.edit',$supplier->id) }}" title="Edit Category"><i data-feather="edit"></i></a>
                                                    @endif
                                                    @if (auth()->user()->has_access_to_route('suppliers.destroy'))
                                                        <a class="nav-link" href="javascript:void(0)" onclick="delete_one_category({{$supplier->id}})" title="Delete Category"><i data-feather="trash"></i></a>
                                                    @endif
                                                @endif
                                            </nav>                                        
                                        </td>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('suppliers.index') }}";
    ...
    <script>
        ...
        function delete_category(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('suppliers.delete')}}",'',selected_pages);
                });
        ...
        function delete_one_category(id){
            ...
            $('#btnDelete').on('click', function() {
                post_form('{{route('suppliers.delete')}}','',id);
            });
        }
    </script>
@endsection
