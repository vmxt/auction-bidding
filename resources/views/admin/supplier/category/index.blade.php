            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-5">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Supplier Categories</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Manage Supplier Categories</h4>
            </div>
        </div>
                        ...
                        <table class="table mg-b-0 table-light table-hover">
                            ...
                            @forelse($categories as $category)
                                <tr id="row{{$category->id}}" class="row_cb">
                                    ...
                                    <td>
                                        @if($category->trashed())
                                            @if (auth()->user()->has_access_to_route('supplier-categories.restore'))
                                                <nav class="nav table-options">
                                                    <a class="nav-link" href="{{route('supplier-categories.restore',$category->id)}}" title="Restore this category"><i data-feather="rotate-ccw"></i></a>
                                                </nav>
                                            @endif
                                        @else
                                            <nav class="nav table-options">
                                                @if (auth()->user()->has_access_to_route('supplier-categories.edit'))
                                                    <a class="nav-link" href="{{ route('supplier-categories.edit',$category->id) }}" title="Edit Category"><i data-feather="edit"></i></a>
                                                @endif
                                                @if (auth()->user()->has_access_to_route('supplier-categories.delete'))
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('supplier-categories.index') }}";
    ...
    <script>
        ...
        function delete_category(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('supplier-categories.delete')}}",'',selected_pages);
                });
        ...
        function delete_one_category(id){
            ...
            $('#btnDelete').on('click', function() {
                post_form('{{route('supplier-categories.delete')}}','',id);
            });
        }
    </script>
@endsection
