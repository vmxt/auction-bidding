                    ...
                    <div class="tab-pane fade @if(!$showTab2) show active @endif" id="home" role="tabpanel" aria-labelledby="home-tab">
                        ...
                        <form class="col-md-6" method="POST" action="{{ route('account.update', $user->id) }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="firstname" class="form-control" required value="{{ old('firstname', $user->first_name) }}">
                                @hasError(['inputName' => 'firstname'])
                                @endhasError
                            </div>
                            <div class="form-group">
                                <label class="d-block">Last Name <span class="tx-danger">*</span></label>
                                <input type="text" name="lastname" class="form-control" required value="{{ old('lastname', $user->last_name) }}">
...
@section('customjs')
    <script>
        ...
        $("#user_image").change(function(evt) {
            ...
            imageName = $('#img_name').html();
            logoPath = $('#userLogo').attr('src');
            ...
            let requiredWidth = "{{ env('USER_LOGO_WIDTH') }}";
            let requiredHeight =  "{{ env('USER_LOGO_HEIGHT') }}";

            validate_files(files, readURL, maxSize, validateFileTypes, requiredWidth, requiredHeight, remove_logo_value);
        });

        function remove_logo_value()
        {
            $('#user_image').val('');
            $('#img_name').html(imageName);
            $('#userLogo').attr('src', logoPath);
        }
    </script>
@endsection
