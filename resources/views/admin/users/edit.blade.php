        ...
        <div class="col-lg-6">
            <form action="{{ route('users.update',$user->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="fname" id="fname" value="{{ old('fname', $user->first_name)}}" class="form-control @error('fname') is-invalid @enderror" required>
                    @hasError(['inputName' => 'fname'])
                    @endhasError
                </div>
                <div class="form-group">
                    <label class="d-block">Last Name *</label>
                    <input type="text" name="lname" id="lname" value="{{ old('lname', $user->last_name)}}" class="form-control @error('lname') is-invalid @enderror" required>
                    @hasError(['inputName' => 'lname'])
                    @endhasError
                </div>
                <div class="form-group">
                    <label class="d-block">Email *</label>
                    <input type="email" name="email" id="email" value="{{ old('email', $user->email)}}" class="form-control @error('email') is-invalid @enderror" required pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$">
